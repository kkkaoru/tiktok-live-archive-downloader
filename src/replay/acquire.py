...
from replay.jobs import already_complete, prepared_download_once
from replay.recordings import Notice, UnavailableReplay, recording_filename
...
def acquire_one(api: TikTokAPI, *, notice: Notice, options: DownloadOptions) -> bool:
    def prepare() -> tuple[Path, Callable[[], None]]:
        # Both filename and URL resolution occur after locking and deduplication.
        replay = api.resolve(notice)
        if not replay.available or replay.media_url is None:
            raise UnavailableReplay("Recording is currently unavailable.")
        output = options.output / recording_filename(replay)
    ...
    return prepared_download_once(
        root=options.store.parent / "jobs", replay_id=notice.replay_id, prepare=prepare
    )
